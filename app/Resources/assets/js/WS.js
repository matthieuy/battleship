...
let WS = function (uri) {
  ...
  let Socket = function (uri) {
    ...
    this._onResult = function (topicName, result) {
      let that = this
      for (let dataName in result) {
        let obj = result[dataName]
  ...
  Socket.prototype.connect = function () {
    ...
    that.on('socket/connect', function () {
      for (let topicName in that._subscribers) {
  ...
  Socket.prototype.callRPC = function (entryPoint, data, cb) {
    if (!this._session) {
      ...
      if (this._tryRPC >= 15) {
        ...
        return setTimeout(function () {
          self.callRPC(entryPoint, data, cb)
        }, 1000)
      }
    }
    ...
    if (typeof data !== 'undefined') {
      Object.assign(data, this._dataDefault)
    } else {
